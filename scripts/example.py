...
def forward_pass(images, net, transformer, batch_size=None):
	...
	global time_to_classify
	time_to_classify = round((end-start),4)
...
def restructer_file(filename):

	f = open(filename,'r')
	tempFile = 'Final_' + filename
	t = open(tempFile,'w')
...
def classify(caffemodel, deploy_file, image_files,
    ...
    for index, classification in enumerate(classifications):
        ...
        fl = open("output_new.txt","a")
	fl.write("{},".format(time_to_classify))
	for label, confidence in classification:
            print '{},{:9.4%},"{}"'.format(time_to_classify,confidence/100.0, label)
	    fl.write('{:9.4%}{},{},{}'.format(confidence/100.0,"",label,""))
	fl.write("\n")
	fl.close()
        print "Hello writing the results to a file"
   # restructer_file("output_new.txt");
